		...
		<div class="col-md-10 col-sm-12 col-xs-12">
			<div class="x_panel">
				...
				@endif
				 >    {{__('ui.add')}} </a> 
					<div class="x_title">
							<h2 style="padding-bottom:5px">Nationalities</h2>
			...
			<div class="x_content">
				...
				<tbody>
					...
					<tr>
						<td> {{$doc_record->code}}</td>
						<td> {{$doc_record->name}}</td>
						<td> {{$doc_record->nationality}}</td>
						
						

						<td class="text-center">
							  <form action="{{'/nationality/'.$nationality->id}}" method="post">
							{{csrf_field()}}
							{{method_field('delete')}}
								<div class=" " role="group">
									<a class="btn btn-primary" href="{{'/nationality/'.$nationality->id.'/edit'}}">{{__('ui.edit')}}</a>
