			  ...
			  <table  id="myTable" class="table table-condensed table-hover table-striped" border="1px solid">
				...
				<tbody>
					...
					<tr>
						...
						<td> {{$doctor_list->pass}}</td>
						<td> {{$doctor_list->status}}</td>
						<td> 
						...
						<a onclick="return confirm('Are you sure?');" class="btn btn-danger" href="{{'/delete/'.$doctor_list->id}}"><i class="fa fa-warni"></i>{{__('ui.delete')}}</a>  
						@elseif($doctor_list->pass == "Yes")
						<a style="border-left:2px solid grey" disabled class="btn btn-primary" href="#"><i class="fa fa-check"></i>Pass</a> 
						...
						@elseif(Auth::user()->status == 1)
							...
							@if($doctor_list->pass == "Yes")
						
						<a style="border-left:2px solid grey" class="btn btn-primary" href="{{'checklist/'.$doctor_list->id.'/create'}}"><i class="fa fa-check"></i>Check List</a> 
	
							@elseif($doctor_list->status == "Approved")
						<center><a id="printButton" href="{{'printcertificate/'.$doctor_list->reg_no.'/certificate'}}"  class="btn btn-primary  pull-right"><i class="fa fa-print"></i> {{__('ui.print')}}</a></center>
